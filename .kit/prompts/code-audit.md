...
(`cmd/`, `internal/`, the root package, `command/`, `fs/`, `network/`, `interp/`, `builtins/`, `transform/`, `parser/`, `sandbox/`, `sqlite/` for this repo).
...
1. **Map the repo first**:
   ...
   - Read `AGENTS.md`, `README.md`, and any `pkg/*/doc.go` to understand the
     ...
     extension surface)
   - Note the public Go API surface (root package + each top-level subpackage) and any documented invariants
...
4. **Check concerns / boundary violations**:
   - **SDK leakage**: grep the public packages (root + subpackages) for imports of `internal/...` types
